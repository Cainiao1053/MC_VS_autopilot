...
function FreeFlight()
    ...
    info = "Select from Minimap";
    term.setCursorPos(w/2-string.len(info)/2,10);
    term.write(info);
    local evn,but,cursx,cursyyy = os.pullEvent("mouse_click");
    if cursyyy == 5 then
        FreeFlightSelect()
    ...
    elseif cursyyy == 6 then
        ...
        while quit == 0 do
            ...
            local evn,but,cursx,cursyyy = os.pullEvent("mouse_click");
            if cursyyy>=5 and cursyyy<=16 then
                if Route[cursyyy+12*(page-1)-4]~=nil then
                    openRoute = io.open(Route[cursyyy+12*(page-1)-4],"r");
                    j = 1;
                    for lines in io.lines(Route[cursyyy+12*(page-1)-4]) do
    ...
    elseif cursyyy == 9 then
        ...
        evn,but,cursx,cursy = os.pullEvent("mouse_click");
    elseif cursyyy == 10 then
        if c ~= nil then
            chatExit = 0;
            xfreeSelected = nil;
            zfreeSelected = nil;
            chatSelectIn = math.random(2,11)*math.random(2,11)*math.random(2,11)*math.random(2,11);
            chatSelectOut = math.random(2,11)*math.random(2,11)*math.random(2,11)*math.random(2,11);
            chatOwner = nil;
            info = "Chat "..chatSelectIn.." to Enter Minimap Select";
            term.setCursorPos(w/2-string.len(info)/2,13);
            term.write(info);
            info = "Chat "..chatSelectOut.." to Quit";
            term.setCursorPos(w/2-string.len(info)/2,14);
            term.write(info);
            info = "Share points from minimap after registeration";
            term.setCursorPos(w/2-string.len(info)/2,15);
            term.write(info);
            while chatExit == 0 do
                local event, chatUser, chatMessage, uuid, isHidden = os.pullEvent("chat");
                if chatOwner==nil and tonumber(chatMessage) == chatSelectIn then
                    chatOwner = chatUser;
                    c.sendMessageToPlayer("Minimap Selection Start", chatOwner, "Cainiao Airline");
                elseif tonumber(chatMessage) == chatSelectOut then
                    chatExit = 1;
                    c.sendMessageToPlayer("Quit Selection", chatUser, "Cainiao Airline");
                end
                if chatOwner ~= nil and chatUser == chatOwner then
                    if string.find(chatMessage, "xaero%-waypoint") ~= nil then
                        ...
                        xfreeSelected = tonumber(chatLocation[2]);
                        zfreeSelected = tonumber(chatLocation[4]);
                        if type(zfreeSelected) == "number" then
                            freepointAdd = freepointAdd + 1;
                            xfree[freepointAdd] = xfreeSelected;
                            zfree[freepointAdd] = zfreeSelected;
                            MODE = 3;
                        sleep(0.4);
                        c.sendMessageToPlayer("Point Added", chatOwner, "Cainiao Airline");
                    else
                        c.sendMessageToPlayer("Can't add point na you FAILURE", chatOwner, "Cainiao Airline");
                        chatExit = 1;
                    end
                    end
                end
            ...
            info = "Chatbox not installed ya you FAILURE";
            term.setCursorPos(w/2-string.len(info)/2,15);
            term.write(info);
            sleep(1);
        end
...
function FlyingPanel() --GUI during flight
    ...
    term.setBackgroundColor(colors.white);
    info = "Height: "..math.floor(cruiseheight);
    term.setCursorPos(w/2-string.len(info)/2,5);
    term.write(info);
    term.setCursorPos(w/2-10,5);
    term.write("-");
    term.setCursorPos(w/2+10,5);
    term.write("+");
    term.setCursorPos(w/2-13,5);
    term.write("--");
    term.setCursorPos(w/2+13,5);
    term.write("++");
    term.setCursorPos(17,6);
    ...
    if cursyp == 5 then
        if cursxp >=(w/2-14) and cursxp<=(w/2-12) then
            cruiseheight = cruiseheight - 40;
            htgt = cruiseheight;
        elseif cursxp >=(w/2-11) and cursxp<=(w/2-9) then
            cruiseheight = cruiseheight - 5;
            htgt = cruiseheight;
        elseif cursxp >=(w/2+9) and cursxp<=(w/2+11) then
            cruiseheight = cruiseheight + 5;
            htgt = cruiseheight;
        elseif cursxp >=(w/2+12) and cursxp<=(w/2+14) then
            cruiseheight = cruiseheight +40;
            htgt = cruiseheight;
        end
        if cruiseheight>310 then
            cruiseheight = 310;
            htgt = cruiseheight;
        end
